...
ms:mtpsurl: https://technet.microsoft.com/ko-kr/library/JJ945599(v=OCS.15)
ms:contentKeyID: 52057014
ms.date: 08/03/2014
...
The Lync Server 2013 영구 채팅 Resource Kit tools help to make routine tasks easier for IT administrators who deploy and manage Lync Server 2013 영구 채팅 서버. In addition to installation instructions, this topic describes the purpose of each tool, and examples of its use.

## Installation of the Resource Kit Tools

To install the Lync Server 2013, 리소스 키트 도구, download **PersistentChatReskit.msi**. Run **PersistentChatReskit.msi** to do a simple installation. The .msi installs all the tools in the following path: \\**Program Files\\ Microsoft Lync Server 2013\\Persistent Chat Server Resource Kit**. Tools that are self-contained executables are in this folder. Tools that also have files are in their own subfolders.


> [!IMPORTANT]
> After installing the Lync Server 2013, 리소스 키트 도구, you must install <STRONG>PsExec.exe</STRONG> and copy <STRONG>PsExec.exe</STRONG> to the following path: \<STRONG>Program Files\ Microsoft Lync Server 2013\Persistent Chat Server Resource Kit\ChatStressTool</STRONG>. If you do not copy <STRONG>PsExec.exe</STRONG>, the 영구 채팅 Stress Tool will throw an error exception, and not perform correctly. Make sure that you meet this prerequisite requirement prior to running the tool. For details about installing <STRONG>PsExec.exe</STRONG>, see <A href="http://go.microsoft.com/fwlink/p/?linkid=282246">http://go.microsoft.com/fwlink/p/?LinkId=282246</A>.



## Supported Environments

For optimal performance, the Lync Server 2013, 리소스 키트 도구 should be installed in the same environment and with the same specifications that are required for Lync Server 2013.

## Resource Kit Tools Overview

Here are the tools that are provided in the Lync Server 2013 영구 채팅 Resource Kit. The following section provides a description of each tool, including requirements and example usage.
...
The AffCheck tool confirms that the 영구 채팅 back-end database user and group affiliation records match that of Active Directory 도메인 서비스.

## Requirements

The tool is installed with the PersistentChatResKit installer on a domain joined machine.

The user account under which the tool is run must have Read access to the 영구 채팅 back-end database and Active Directory Domain Services.
...
The PersistentChatMonitoringSummary tool moves 영구 채팅 monitoring information from the monitoring database into a specified CSV log file.

The CSV file will contain a breakdown of 영구 채팅 sessions by number of total sessions, successful sessions, unexpected failures, expected failures, and a breakdown of the unexpected failures by diagnostic ID, number of failures, and failure description.

## Requirements

Install the 영구 채팅 Resource Kit tools on a domain-joined machine that has access to the Monitoring database.
...
The 영구 채팅 Stress tool provides an easy way to simulate usage of 영구 채팅 to test real-world performance, including varied user models to better fit your expected usage scenarios.

## Requirements

Install the 영구 채팅 Resource Kit tools onto a domain-joined machine that has access to the 영구 채팅 back-end database.
...
Create users and channels for use in the stress run:
  ...
  - Create as many users as your user model calls for, enable them for Lync, and set their 영구 채팅 policy to Enabled.
  ...
  - We recommend creating extra stress rooms. You can create 50,000 rooms with the following Windows PowerShell 명령줄 인터페이스 command:
...
6.  Change “ConnectionString” to a connection string for your 영구 채팅 back-end database.
...
The ChatUsageReport tool generates an HTML report of 영구 채팅 service usage.

## Requirements

Install the 영구 채팅 Resource Kit tools on a domain-joined machine that has access to the 영구 채팅 back-end database.

The user account under which the tool is run must have Read access to the 영구 채팅 back-end database.

The file, ChatUsageReport.exe.config, must contain a \<connectionStrings\> section defining the connection string to the 영구 채팅 back-end database. The contents of the default config file are included here, for your reference.
...
ScheduleADSyncForPrincipal is a Microsoft SQL Server 2012 script that must be run directly from within SQL Server Management Studio when connected to the 영구 채팅 back-end database. This script enables you to force 영구 채팅 to synchronize its records of a user with those of Active Directory Domain Services, rather than waiting for the scheduled synchronization time.

## Requirements

The user account under which the script is run must have owner access to the 영구 채팅 back-end database.
