...
TOCTitle: 미디어 바이패스 개요
ms:assetid: 9ea090b3-f607-46f7-97dd-2510052524e5
ms:mtpsurl: https://technet.microsoft.com/ko-kr/library/Gg412740(v=OCS.15)
ms:contentKeyID: 49304546
ms.date: 08/24/2015
mtps_version: v=OCS.15
ms.translationtype: HT
---

# Lync Server 2013의 미디어 바이패스 개요

 

_**마지막으로 수정된 항목:** 2012-09-21_

미디어 바이패스는 배포할 중재 서버 수를 최소화할 때 유용합니다. 일반적으로 중재 서버 풀은 중앙 사이트에 배포되며 분기 사이트에서 게이트웨이를 제어합니다. 미디어 바이패스를 사용하도록 설정하면 분기 사이트에서 클라이언트의 PSTN(공중 전화망) 통화에 대한 미디어가 해당 사이트에서 게이트웨이를 통과해 바로 흐르도록 할 수 있습니다. 분기 사이트에서 클라이언트의 PSTN 통화가 적합한 게이트웨이로 라우팅되도록 Lync Server 2013 아웃바운드 통화 경로 및 Enterprise Voice 정책을 제대로 구성해야 합니다.
...
Enterprise Voice 구조를 설정했으면 미디어 바이패스에 대한 계획은 간단합니다.
...
사용자가 PSTN에 전화를 걸면 중재 서버에서 클라이언트 서브넷의 바이패스 ID와 게이트웨이 서브넷의 바이패스 ID를 비교합니다. 두 개의 바이패스 ID가 일치할 경우 통화에 미디어 바이패스가 사용됩니다. 바이패스 ID가 일치하지 않을 경우에는 통화에 대한 미디어가 중재 서버를 통해 통과해야 합니다.

사용자가 PSTN을 통해 전화를 받으면 사용자의 클라이언트가 클라이언트의 바이패스 ID와 PSTN 게이트웨이의 바이패스 ID를 비교합니다. 두 개의 바이패스 ID가 일치하면 미디어가 중재 서버를 바이패스하고 게이트웨이에서 클라이언트로 바로 통과됩니다.

Lync 2010 이상의 클라이언트 및 장치만 중재 서버와의 미디어 바이패스 상호 작용을 지원합니다.
